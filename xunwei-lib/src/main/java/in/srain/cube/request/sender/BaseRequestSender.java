...
public abstract class BaseRequestSender implements IRequestSender {
    ...
    public void setup() throws IOException {
    	//上传cookie
        String cookie = CubeCookieManager.getCookie(mRequest.getRequestData().getRequestUrl());
    ...
    public void getResponse(StringBuilder sb) throws IOException {
        ...
        if (status == HttpURLConnection.HTTP_OK) {
            ...
            //把服务端返回的cookie保存起来
            CubeCookieManager.setCookies(mRequest.getRequestData().getRequestUrl(), mHttpURLConnection.getHeaderFields());
